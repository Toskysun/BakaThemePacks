...
const CONTRAST_PAIRS = [
    ...
    ['--theme-text', '--theme-surface-strong', 4.5],
    ['--theme-header-text', '--theme-header-bg', 4.5],
    ['--theme-sidebar-text', '--theme-sidebar-bg', 4.5],
    ['--theme-player-text', '--theme-player-bg', 4.5],
    ['--theme-panel-text', '--theme-panel-bg', 4.5],
    ['--theme-popover-text', '--theme-popover-bg', 4.5],
    ['--theme-text-on-primary', '--theme-primary', 4.5],
    ['--theme-text-secondary', '--theme-surface-strong', 3],
    ['--theme-sidebar-text-secondary', '--theme-sidebar-bg', 3],
    ['--theme-player-text-secondary', '--theme-player-bg', 3],
    ['--theme-panel-text-secondary', '--theme-panel-bg', 3],
    ['--theme-popover-text-secondary', '--theme-popover-bg', 3],
];

export function auditThemeContrast(tokens) {
    const failures = [];
    for (const [foregroundToken, backgroundToken, requiredRatio] of CONTRAST_PAIRS) {
        if (!tokens.has(foregroundToken) || !tokens.has(backgroundToken)) {
            failures.push(`缺少可读性校验所需 token: ${foregroundToken} / ${backgroundToken}`);
            continue;
        }
        const foreground = resolveColour(tokens.get(foregroundToken), tokens);
        const background = resolveColour(tokens.get(backgroundToken), tokens);
        if (!foreground || !background) {
            failures.push(`${foregroundToken} / ${backgroundToken} 使用了无法静态校验的颜色格式`);
            continue;
        }
        const ratio = minimumContrast(foreground, background);
        if (ratio + 0.001 < requiredRatio) {
            failures.push(`${foregroundToken} / ${backgroundToken} 对比度 ${ratio.toFixed(2)}:1，最低要求 ${requiredRatio}:1`);
        }
    }
    return failures;
}
