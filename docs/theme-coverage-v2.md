...
| 标题栏 / 搜索 / 历史 | `header-*`, `popover-*` | `Header`, `SearchHistory` |
...
| 音乐详情 / 歌词工具 | `detail-text*`, `detail-surface*`, `detail-border`, `detail-accent` | MusicDetail, Lyric；封面流光舞台由客户端拥有 |
...
3. 详情页封面流光背景、播放栏 auto-hide 及 glass 播放栏封面动态取色均为产品行为；主题只覆盖这些区域的语义表面与文字。
